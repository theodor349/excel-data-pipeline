...
def _right():
    return pd.DataFrame({"id": [1, 2], "extra": ["x", "y"]})
...
def test_merge_left_join_on_preserves_left_rows():
    ...
    assert result.loc[result["id"] == 3, "extra"].isna().all()


def test_merge_inner_join_left_on_right_on():
    left = pd.DataFrame({"lid": [1, 2, 3], "val": ["a", "b", "c"]})
    right = pd.DataFrame({"rid": [1, 2], "extra": ["x", "y"]})
...
def test_append_concatenates_and_reindexes():
    top = pd.DataFrame({"a": [1, 2], "b": [3, 4]})
    bottom = pd.DataFrame({"a": [5], "b": [6]})
...
def test_append_works_with_reordered_columns():
    top = pd.DataFrame({"a": [1], "b": [2], "c": [3]})
    bottom = pd.DataFrame({"b": [20], "c": [30], "a": [10]})
    result = append(top, bottom)
    assert len(result) == 2
...
def test_append_raises_on_column_mismatch():
    top = pd.DataFrame({"a": [1], "b": [2]})
    bottom = pd.DataFrame({"a": [3], "c": [4]})
