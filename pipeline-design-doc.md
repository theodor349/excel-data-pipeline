...
Writes structured logs to a timestamped log file in the output folder. Each run creates a new file so that history is preserved and runs do not overwrite each other.
...
Each query file exposes two functions: `load()` returns a named dict of source DataFrames; `run(data)` transforms them and returns the output dict. This separation allows the test runner to substitute fixture data for real sources without modifying the query logic.
...
Fixture files can be either `.xlsx` or `.csv`. CSV is preferred for simplicity and readability in version control; use `.xlsx` only when column types cannot be preserved in CSV.

Because queries often load from multiple sources, the query is split into two functions: `load()` and `run(data)`. The `load()` function reads all sources and returns a named dict of DataFrames. The `run(data)` function accepts that dict and produces the output. The test runner substitutes `load()` with a version that reads from fixture files, then calls `run(data)` normally.
...
The test runner (`tester.py`) loads each fixture file into a DataFrame, passes the resulting dict to `run(data)`, and compares each output sheet against its expected file. The user's job is to fill in the fixture files with a small representative sample (e.g. 10–20 rows) and the expected output for that sample.
...
4. Agent creates expected CSV file(s)
...
- [ ] Implement `engine/tester.py` with fixture comparison (supports `.csv` and `.xlsx`, substitutes `load()` with fixture version)
...
- [ ] Write `pytest` tests for all seeded functions under `functions/tests/`
